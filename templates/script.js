document.getElementById('cadastroForm').addEventListener('submit', function (event) {
    ...
    // permite o envio apenas se todas as validações passarem
    if (camposValidos.every(Boolean)) {
        alert('Cadastro realizado com sucesso!');
        // this.submit(); //IMPLEMENTAR BACKEND
    } else {
        showError('formError', 'Por favor, corrija os campos destacados');
    }
});
...
function verificarForcaSenha() {
    ...
    const isLong = senha.length >= 8;
    
    let strength = 0;
    if (senha.length > 0) strength = 1;
    if (senha.length >= 4) strength = 2;
    if ((hasLower || hasUpper) && hasNumber && senha.length >= 6) strength = 3;
    if (hasLower && hasUpper && hasNumber && hasSpecial && isLong) strength = 4;
    
    //atualiza visualização
    const colors = ['#ff4d4d', '#ffa64d', '#ffff4d', '#4dff4d'];
    for (let i = 0; i < strength; i++) {
        sections[i].style.backgroundColor = colors[strength - 1];
    }
}